...
SLURM_CPU_SMALL_PARTITION ?= ${SLURM_CPU_PARTITION}
SLURM_CPU_LARGE_PARTITION ?= ${SLURM_CPU_PARTITION}
SLURM_GPU_SMALL_PARTITION ?= ${SLURM_GPU_PARTITION}
SLURM_GPU_LARGE_PARTITION ?= ${SLURM_GPU_PARTITION}

ifneq (${SLURM_GPUS},0)
  ifeq (${NR_OF_NODES},1)
    SLURM_PARTITION ?= ${SLURM_GPU_SMALL_PARTITION}
  else
    SLURM_PARTITION ?= ${SLURM_GPU_LARGE_PARTITION}
  endif
  SLURM_TIME      ?= ${SLURM_MAX_GPU_TIME}
  SLURM_GRES      ?= ${SLURM_GPU_GRES}:${SLURM_GPUS}
else
  ifeq (${NR_OF_NODES},1)
    SLURM_PARTITION ?= ${SLURM_CPU_SMALL_PARTITION}
  else
    SLURM_PARTITION ?= ${SLURM_CPU_LARGE_PARTITION}
  endif
